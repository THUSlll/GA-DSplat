...
class ViewSamplerBoundedV3Cfg:
    ...
    random_num_view_sample:bool = False


class ViewSamplerBoundedV3(ViewSampler[ViewSamplerBoundedV3Cfg]):

    def set_batch_seed(self, seed: int):
        """Set a batch-level random seed to ensure consistent random_num_views within a batch"""
    ...
    def sample(
        ...
        if self.cfg.random_num_view_sample and self.stage != "test":
            batch_seed = (self.global_step * 1000) % (2**32)
            
            temp_rng = random.Random(batch_seed)
            random_num_views = temp_rng.randint(2, self.cfg.num_context_views)
            random_num_views = int( random_num_views / 2 ) * 2
            multiple = int(random_num_views / 2)
            max_distance_between_context_views = int(self.cfg.max_distance_between_context_views * multiple)
            min_distance_between_context_views = int(self.cfg.min_distance_between_context_views * multiple)
        else:
            random_num_views = None
            multiple = int(self.cfg.num_context_views / 2)
            max_distance_between_context_views = int(self.cfg.max_distance_between_context_views * multiple)
            min_distance_between_context_views = int(self.cfg.min_distance_between_context_views * multiple)
        ...
        if self.stage == "test":
            ...
            max_context_gap = max_distance_between_context_views
            min_context_gap = min_distance_between_context_views
        ...
        if self.stage != "test" and self.cfg.target_gap_warm_up_steps > 0:
            ...
            max_target_gap = max_distance_between_context_views
