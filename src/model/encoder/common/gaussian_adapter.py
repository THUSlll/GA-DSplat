...
class GaussianAdapter(nn.Module):
    ...
    def forward(
        ...
        if pooling_mask is not None:
            pooling_mask = pooling_mask.float().unsqueeze(-1).unsqueeze(-1) # (B, V, G, 1)
        else: pooling_mask = 1
        ...
        return Gaussians(
            ...
            opacities=opacities * pooling_mask,
...
class GaussianUnproject(nn.Module):
...
class GaussianProject(nn.Module):
    ...
    def forward(
        ...
        means_homo = torch.cat([means, ones], dim=-1)  # [..., 4]
        # 世界到相机
        means_homo = means_homo.unsqueeze(2).expand(B, V, V, *means_homo.shape[2:])

        extrinsics_exp = extrinsics.unsqueeze(1).expand(B, V, V, *extrinsics.shape[2:])
        intrinsics_exp = intrinsics.unsqueeze(1).expand(B, V, V, *intrinsics.shape[2:])

        cam_points = torch.matmul(extrinsics_exp.inverse(), means_homo.unsqueeze(-1))[..., :3, 0]  # [..., 3]
        # 或者直接用齐次投影
        proj = torch.matmul(intrinsics_exp, cam_points.unsqueeze(-1))[..., :, 0]  # [..., 3]
