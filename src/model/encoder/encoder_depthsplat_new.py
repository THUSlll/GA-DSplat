...
from .common.gaussian_adapter import GaussianAdapter, GaussianAdapterCfg, GaussianUnproject, GaussianProject
...
from .GGN.gaussian_graph import GGNLinearLayer, GGNPoolingLayer
...
class EncoderDepthSplat(Encoder[EncoderDepthSplatCfg]):
    def __init__(self, cfg: EncoderDepthSplatCfg) -> None:
        ...
        # self.gaussian_strength = nn.Sequential(
        ...
        # self.ggn_offset_head = nn.Sequential(
        #         nn.Conv2d(in_channels, 2,
        #                   3, 1, 1, padding_mode='replicate'),
        #         nn.GELU(),
        #         nn.Conv2d(2,
        #                   2, 3, 1, 1, padding_mode='replicate')
        #     )
        ...
        self.ggnPooling = GGNPoolingLayer(0.1)
    ...
    def forward(
        ...
        pixel_offset = self.gaussian_head(out)
        #pixel_offset = self.ggn_offset_head(out)
        pixel_offset = rearrange(pixel_offset, "(b v) c h w -> b v c h w", b=b, v=v)
        
        raw_pixel_offset = rearrange(
            pixel_offset, "b v c h w -> b v (h w) c"
        )

        raw_pixel_offset = raw_pixel_offset[..., :2]

        pixel_offset = rearrange(
            raw_pixel_offset,
            "... (srf c) -> ... srf c",
        ...
        offset_xy = pixel_offset.sigmoid()
        ...
        if self.cfg.supervise_intermediate_depth and len(depth_preds) > 1:
            ...
            context_extrinsics = torch.cat(
                [context["extrinsics"]] * num_depths, dim=0)
            context_intrinsics = torch.cat(
                [context["intrinsics"]] * num_depths, dim=0)
            xy_ray = torch.cat(
                [xy_ray] * num_depths, dim=0)
        ...
        if self.cfg.supervise_intermediate_depth and len(depth_preds) > 1:
            ...
            feat = rearrange(out, "(b v) c h w -> b v (h w) c", b = b, v = v)

            # feat, A = self.ggnLinear(feat, xy, context_extrinsics)
        
        else:
            feat = rearrange(out, "(b v) c h w -> b v (h w) c", b = b, v = v)

            # feat, A = self.ggnLinear(feat, xy, context["extrinsics"])



        feat = rearrange(feat, "b v (h w) c -> (b v) c h w", h = h, w = w)

        gaussians = self.gaussian_head(feat)

        raw_gaussians = rearrange(gaussians, "(b v) c h w -> b v (h w) c", b = b, v = v)   

        raw_gaussians = raw_gaussians[..., 2:]

        pooling_mask = None

        # pooling_feat, pooling_means, pooling_mask = self.ggnPooling(raw_gaussians, means.squeeze(3, 4), xy, A)
