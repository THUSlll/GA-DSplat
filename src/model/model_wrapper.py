...
class ModelWrapper(LightningModule):
    ...
    def run_full_test_sets_eval(self) -> None:
        ...
        overall_eval_time = time() - start_t
    ...
    def configure_optimizers(self):
        freeze_steps = self.optimizer_cfg.freeze_step

        ggn_params = [p for n, p in self.named_parameters() if "triplane" in n or "ggn" in n]
        pretrained_params = [p for n, p in self.named_parameters() if "pretrained" in n]
        other_params = [p for n, p in self.named_parameters() if "triplane" not in n and "pretrained" not in n and "ggn" not in n]

        # 简化逻辑：总是使用多参数组结构，这样与保存的checkpoint兼容性更好
        # 在训练过程中通过设置requires_grad来控制哪些参数被训练
        optimizer = torch.optim.AdamW(
            [
                {"params": pretrained_params, "lr": self.optimizer_cfg.lr_monodepth},
                {"params": other_params, "lr": self.optimizer_cfg.lr},
                {"params": ggn_params, "lr": self.optimizer_cfg.lr_GGN},
            ],
        ...
        scheduler = torch.optim.lr_scheduler.OneCycleLR(
            optimizer,
            [self.optimizer_cfg.lr_monodepth, self.optimizer_cfg.lr, self.optimizer_cfg.lr_GGN],
        ...
        if self.global_step < freeze_steps:
            for param in other_params + pretrained_params:
                param.requires_grad = False
            for param in ggn_params:
    ...
    def on_after_backward(self):
        ...
        if self.global_step == 0:
            ...
            if no_grad_params:
                print(f"⚠️ {len(no_grad_params)} parameters have NO gradient:")
                for name in no_grad_params:
                    print(f"   - {name}")
            else:
                print("✅ All parameters have gradients.")

        #     # 1. 首先对门控网络进行特别严格的裁剪（这是失控的源头）
        # torch.nn.utils.clip_grad_norm_(
        #     self.encoder.triplane_fuse.gate.parameters(), 
        #     max_norm=0.2  # 门控网络需要最严格的控制
        # )
        
        # # 2. 对三平面生成器进行中等裁剪
        # torch.nn.utils.clip_grad_norm_(
        #     self.encoder.triplane_generator.parameters(),
        #     max_norm=0.4
        # )
        ...
        #     if "gate" not in n and "triplane_generator" not in n],
        #     max_norm=0.6
        # )
    ...
    def on_train_batch_start(self, batch, batch_idx, dataloader_idx=0):
        ...
        if self.global_step == freeze_steps:
            print(f"🔓 到达 step {self.global_step}，解冻所有参数以开始联合训练")

            # 解冻所有参数
            for param in self.parameters():
                param.requires_grad = True

        if self.global_step == 0 or self.global_step == freeze_steps:
            print(f"Step {self.global_step}: Optimizer has {len(self.optimizers().param_groups)} param groups")
            for i, g in enumerate(self.optimizers().param_groups):
                print(f"  Group {i}: {len(g['params'])} params, lr = {g['lr']:.2e}")
